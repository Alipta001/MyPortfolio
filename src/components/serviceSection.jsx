...
const iconMap = { Code, Layout, Smartphone, Zap };
...
function ServiceCard({ service, index }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay: index * 0.1, duration: 0.8 }}
      className="service-card relative group h-[400px] md:h-[450px] rounded-[3rem] overflow-hidden border border-white/5 bg-[#0a0a0a]"
    >
      {/* 1. BACKGROUND IMAGE LAYER */}
      <div className="absolute inset-0 z-0">
        <img 
          src={service.image} 
          alt={service.title}
          className="w-full h-full object-cover max-md:grayscale-0 max-md:scale-110 md:grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-60 group-hover:scale-110 transition-all duration-700"
        />
        {/* Dark Overlays for Text Readability */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/80 to-transparent" />
        
        {/* Mobile: Always 100% visible gradient overlay | Desktop: Opacity 0 shifting to 100 on hover */}
        <div className={`absolute inset-0 bg-gradient-to-br ${service.color} mix-blend-overlay max-md:opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-700`} />
      </div>
      ...
      <div className="relative z-10 h-full p-8 md:p-12 flex flex-col justify-between">
        ...
        {/* Top Section: Icon and Index */}
        <div className="flex justify-between items-start">
          {/* Mobile: Cyan border and color active by default | Desktop: Triggers only on hover */}
          <div className="w-14 h-14 rounded-2xl bg-white/5 backdrop-blur-xl border max-md:border-cyan-500/50 max-md:text-cyan-400 md:border-white/10 text-white md:group-hover:border-cyan-500/50 md:group-hover:text-cyan-400 transition-all duration-500">
            <Icon className="w-6 h-6" />
          </div>
          <span className="text-5xl font-black italic max-md:opacity-30 opacity-10 md:group-hover:opacity-30 transition-opacity duration-500">
            0{index + 1}
          </span>
        </div>
      ...
      {/* Decorative Glow on Hover (Always visible on mobile) */}
      <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-cyan-500/20 blur-[100px] max-md:opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-700" />
    </motion.div>
  );
}